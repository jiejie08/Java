...
public class ExecuteClient implements Runnable {
    ...
    public void run() {//实现功能,处理客户端请求
        try {
            //1、获取输入流不断的读取用户发来的信息
            InputStream clientInput = this.client.getInputStream();
            Scanner scanner = new Scanner(clientInput);
            while (true) {
                String line = scanner.nextLine();//在此等待用户输入
    ...
    private void privateChat(String userName, String message) {
        ...
        if (target != null) {//私聊必须判断这个人在不在,否则就会空指针异常
            this.sendMessage(target, currentUserName + " 对你说： " + message);//当前用户对你说
        }
    }
    ...
    private void register(String userName, Socket client) {
        ...
        ONLINE_USER_MAP.put(userName, client);//将用户信息保存到服务器中
        printOnlineUser();
        sendMessage(this.client, userName + "注册成功!");
    }
    //获取当前用户名（公共）
    private String getCurrentUserName() {
        String currentUserName = "";//定义一个获取当前用户名的变量
        for (Map.Entry<String, Socket> entry : ONLINE_USER_MAP.entrySet()) {//遍历Map里面的Values
            if (this.client.equals(entry.getValue())) {
                currentUserName = entry.getKey();//获取到entry的key了赋值给currentUserName
                break;
            }
        }
    ...
    private static void printOnlineUser() {
        ...
        for (Map.Entry<String, Socket> entry : ONLINE_USER_MAP.entrySet()) {//遍历Map
